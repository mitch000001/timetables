...
func TestEstimationBillingPeriodCreate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			EstimationBillingPeriod{
				...
				BilledDays:                      NewFloat(10).Sub(NewFloat(1.4)).Mul(NewFloat(0.8)),
				CumulatedBilledDays:             10.57,
				EffectiveBillingDegree:          NewFloat(10).Sub(NewFloat(1.4)).Mul(NewFloat(0.8)).Div(NewFloat(10)),
		...
		},
		{
			...
			EstimationBillingPeriod{
				...
				CumulatedSicknessInterest:       1.08,
				BilledDays:                      NewFloat(20).Sub(NewFloat(3.5)).Mul(NewFloat(0.8)),
				CumulatedBilledDays:             10.57,
				EffectiveBillingDegree:          NewFloat(20).Sub(NewFloat(3.5)).Mul(NewFloat(0.8)).Div(NewFloat(20)),
				CumulatedEffectiveBillingDegree: 0.66,
			},
		},
	}
	...
	for _, test := range tests {
		period, err := CreateEstimationBillingPeriod(test.planConfigInput, test.userConfigInput)
